    ...
    try:
        ...
        if is_uvr:
            try:
                ...
                # Check if the uvr_model exists. If not, fallback to an existing model in assets/uvr5_weights/
                uvr_root = os.getenv("weight_uvr5_root", "assets/uvr5_weights")
                model_path = os.path.join(uvr_root, uvr_model + ".pth")
                if not os.path.exists(model_path):
                    candidates = []
                    if os.path.isdir(uvr_root):
                        candidates = [os.path.splitext(f)[0] for f in os.listdir(uvr_root) if f.endswith(".pth")]
                    if candidates:
                        if "HP2_all_vocals" in candidates:
                            uvr_model = "HP2_all_vocals"
                        elif "HP3_all_vocals" in candidates:
                            uvr_model = "HP3_all_vocals"
                        else:
                            uvr_model = candidates[0]
                    else:
                        raise FileNotFoundError(f"Không tìm thấy model UVR nào trong {uvr_root}")
                ...
                is_mdx = (uvr_model == "onnx_dereverb_By_FoxJoy")
                ...
                for f in os.listdir(sep_dir):
                    ...
                    if is_mdx:
                        is_vocal_named = "_main_vocal" in f_lower
                        is_inst_named = "_others" in f_lower
                    else:
                        is_vocal_named = f_lower.startswith("vocal_")
                        is_inst_named = f_lower.startswith("instrument_")
        ...
        if not tmp_path or not os.path.exists(tmp_path):
            ...
            if is_tiktok or is_youtube:
                return (
                    ...
                    "Hãy thử kiểm tra lại URL hoặc tải yt-dlp mới nhất.",
                    (None, None)
                )
